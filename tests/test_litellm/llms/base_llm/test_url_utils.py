"""Tests for the shared URL-encoding helpers.

Directly exercises ``encode_path_segment`` / ``encode_url_path`` and
walks each provider transformation that interpolates a user-controlled
identifier into an outbound URL, asserting that the value is encoded
as an atomic path segment.
"""
...
def _assert_input_is_encoded(url: str, prefix: str) -> None:
    """Shared assertion: after ``prefix`` in ``url`` the sample input
    appears only in its percent-encoded form."""
...
class TestEncodeUrlPath:
    ...
    def test_encodes_query_fragment_and_colon(self):
        ...
        # ``:`` is explicitly encoded to stop ``evil.com:80/x`` being
        # parsed as an authority by a later urljoin.
...
# ---------------------------------------------------------------------------
# Provider-level regressions: confirm identifiers are percent-encoded at
# each URL-construction site.
# ---------------------------------------------------------------------------
...
class TestBedrockCountTokensEncoding:
    def test_endpoint(self):
        ...
        cfg = BedrockCountTokensConfig()
        url = cfg.get_bedrock_count_tokens_endpoint(
            model=SAMPLE_INPUT,
...
class TestRagflowEncoding:
    def test_rejects_dot_segment(self):
        """Ragflow parses entity_id out of the model string; a bare
        ``..`` segment there now raises from encode_path_segment."""
    ...
    def test_query_chars_in_entity_id_are_encoded(self):
        """A non-``..`` entity_id with reserved characters gets encoded."""
        from litellm.llms.ragflow.chat.transformation import RAGFlowConfig
...
# ---------------------------------------------------------------------------
# Additional provider regressions from the follow-up audit sweep.
# ---------------------------------------------------------------------------
...
class TestAzureAIAgentsEncoding:
    def _handler(self):
        # Construct a bare instance without running __init__ (which needs a
        # full config). We're only exercising the _build_* URL methods.
...
class TestVertexVideosEncoding:
    def test_operation_status_rejects_dot_segments(self):
        ...
        # operation_name flows through encode_url_path, which rejects
        # whole-segment ``..``.
        with pytest.raises(ValueError):
...
class TestNvidiaNimEncoding:
    def test_rejects_dot_segment(self):
        ...
        # NIM rerank models have the form "provider/model" (e.g.
        # "nvidia/nv-rerankqa-mistral-4b-v3"), so this uses encode_url_path
        # which rejects whole ".." segments.
        with pytest.raises(ValueError):
            cfg.get_complete_url(
...
class TestHuggingfaceEmbeddingEncoding:
    def test_ids_encoded(self):
        from litellm.llms.huggingface.common_utils import _fetch_inference_provider_mapping  # noqa: F401

        # This module-level helper builds the URL inline; verify that the
        # encoder is actually applied by inspecting a separate helper that
